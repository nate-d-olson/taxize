...
get_uid <- function(sciname, ask = TRUE, verbose = TRUE, rows = NA, modifier = NULL,
  ...
  fun <- function(sciname, ask, verbose, rows, ...) {
    ...
    url <- paste("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=taxonomy&term=",
                 sciname, sep = "")
    ...
    if (length(uid) > 1) {
      if (ask) {
        baseurl <- "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=taxonomy"
